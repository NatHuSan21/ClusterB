...
for k in range(1,25):#25
    ...
    for lam in LAMval:
        ...
        ruta = './restoration/P292B/denoised_P292B_'
        fname = ruta + str(Lambda)  + nameimg 
        out_img_avg_pil.save(fname)

        #Save best image 
        imgPNSRmax_pil = np_to_pil(imgPNSRmax_np)
        fname = './restoration/P292B/BestPnsr_P292B_' + str(Lambda) + nameimg 
    ...
    ruta = './txt292B/'
    name = nameimg.replace('.png','.csv')
    fname = ruta  + 'P292BdenMAXval' + name
