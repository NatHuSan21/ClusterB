...
for k in range(1,25):#25
    ...
    for lam in LAMval:
        ...
        ruta = './restoration/P263B/deblur_P263B_'
        fname = ruta + str(Lambda)  + nameimg 
        out_img_avg_pil.save(fname)

        #Save best image 
        imgPNSRmax_pil = np_to_pil(imgPNSRmax_np)
        fname = './restoration/P263B/BestPnsr_P263B_' + str(Lambda) + nameimg 
    ...
    ruta = './txt263B/'
    name = nameimg.replace('.png','.csv')
    fname = ruta  + 'P263BdebMAXval' + name
