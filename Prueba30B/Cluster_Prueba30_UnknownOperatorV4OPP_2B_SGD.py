...
#listaRep =[3,4,6,9,11,12,15,17,19,22]
listaRep =[5,14,16,18,20,23,24,15,17,19,22]
for k in listaRep:#[:2]:#for k in range(1,25):#25
    ...
    for reg_noi_val in RNSval:
        ...
        def closure():
            ...
            #Lista loss
            #Losslist.append( torch.sum(loss_regularizer),dim=1)
            #print(type(torch.sum(total_loss)))
    ...
    #Guardar datos de gráfica Loss
    yLoss = np.array(plotLoss)
    #ruta = './txt290/'
    ruta = './txt'+ Nprueb +'/'
    name = nameimg.replace('.png','.csv')
    #fname = ruta + 'P290denPNSR' + name
    fname = ruta + 'P'+ Nprueb +'denLoss' + name
    np.savetxt(fname, yLoss.T, delimiter =",",fmt ='% s')
