...
for k in range(1,25):#
    ...
    for lam in LAMval:
        ...
        full_net =  EuclideanDiracColor241B(input_depth, dtype, pad, height=img_height, width= img_width, upsample_mode='bilinear').type(dtype)
        ...
        ruta = './restoration/P241B/denoised_P241B_'
        fname = ruta + str(Lambda)  + nameimg 
        out_img_avg_pil.save(fname)

        #Save best image 
        imgPNSRmax_pil = np_to_pil(imgPNSRmax_np)
        fname = './restoration/P241B/BestPnsr_P241B_' + str(Lambda) + nameimg 
    ...
    ruta = './txt241B/'
    name = nameimg.replace('.png','.csv')
    fname = ruta  + 'P241BdenMAXval' + name
