...
for k in range(1,25):#
    ...
    for lam in LAMval:
        ...
        full_net =  EuclideanDiracColor244B(input_depth, dtype, pad, height=img_height, width= img_width, upsample_mode='bilinear').type(dtype)
        ...
        ruta = './restoration/P244B/denoised_P244B_'
        fname = ruta + str(Lambda)  + nameimg 
        out_img_avg_pil.save(fname)

        #Save best image 
        imgPNSRmax_pil = np_to_pil(imgPNSRmax_np)
        fname = './restoration/P244B/BestPnsr_P244B_' + str(Lambda) + nameimg 
    ...
    ruta = './txt244B/'
    name = nameimg.replace('.png','.csv')
    fname = ruta  + 'P244BdenMAXval' + name
