...
for k in range(1,25):#
    ...
    for lam in LAMval:
        ...
        full_net =  EuclideanDiracColor243B(input_depth, dtype, pad, height=img_height, width= img_width, upsample_mode='bilinear').type(dtype)
        ...
        ruta = './restoration/P243B/denoised_P243B_'
        fname = ruta + str(Lambda)  + nameimg 
        out_img_avg_pil.save(fname)

        #Save best image 
        imgPNSRmax_pil = np_to_pil(imgPNSRmax_np)
        fname = './restoration/P243B/BestPnsr_P243B_' + str(Lambda) + nameimg 
    ...
    ruta = './txt243B/'
    name = nameimg.replace('.png','.csv')
    fname = ruta  + 'P243BdenMAXval' + name
