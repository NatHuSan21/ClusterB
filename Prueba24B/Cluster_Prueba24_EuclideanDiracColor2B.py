...
for k in range(1,25):#
    ...
    for lam in LAMval:
        ...
        full_net =  EuclideanDiracColor242B(input_depth, dtype, pad, height=img_height, width= img_width, upsample_mode='bilinear').type(dtype)
        ...
        ruta = './restoration/P242B/denoised_P242B_'
        fname = ruta + str(Lambda)  + nameimg 
        out_img_avg_pil.save(fname)

        #Save best image 
        imgPNSRmax_pil = np_to_pil(imgPNSRmax_np)
        fname = './restoration/P242B/BestPnsr_P242B_' + str(Lambda) + nameimg 
    ...
    ruta = './txt242B/'
    name = nameimg.replace('.png','.csv')
    fname = ruta  + 'P242BdenMAXval' + name
