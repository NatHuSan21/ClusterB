...
class EuclideanDiracColor241B(nn.Module):
	def __init__(self,input_depth, dtype, pad, height, width, upsample_mode, n_channels=3, act_fun='LeakyReLU', skip_n33d=128, skip_n33u=128, skip_n11=4, num_scales=5, downsample_mode='stride'):
		super(EuclideanDiracColor241B,self).__init__()
		self.net = skip(input_depth, n_channels, num_channels_down = [skip_n33d]*num_scales if isinstance(skip_n33d, int) else skip_n33d,
                                            num_channels_up =   [skip_n33u]*num_scales if isinstance(skip_n33u, int) else skip_n33u,
                                            num_channels_skip = [skip_n11]*num_scales if isinstance(skip_n11, int) else skip_n11, 
                                            upsample_mode=upsample_mode, downsample_mode=downsample_mode,
	...
	def forward(self, input):	
		...
		norm = torch.sqrt(0.00001*torch.ones((self.height,self.width)).type(self.dtype) + torch.maximum(0.00001*torch.ones((self.height,self.width)).type(self.dtype),norm_grad_red_squared + norm_grad_green_squared + norm_grad_blue_squared //
			+ (torch.mul(red_x,green_y)-torch.mul(red_y,green_x)) + (torch.mul(blue_x,red_y)-torch.mul(blue_y,red_x)) + (torch.mul(green_x,blue_y)-torch.mul(green_y,blue_x)))) 
		norm = torch.unsqueeze(norm,dim=0)

		return output,norm	

class EuclideanDiracColor242B(nn.Module):
	def __init__(self,input_depth, dtype, pad, height, width, upsample_mode, n_channels=3, act_fun='LeakyReLU', skip_n33d=128, skip_n33u=128, skip_n11=4, num_scales=5, downsample_mode='stride'):
		super(EuclideanDiracColor242B,self).__init__()
		self.net = skip(input_depth, n_channels, num_channels_down = [skip_n33d]*num_scales if isinstance(skip_n33d, int) else skip_n33d,
                                            num_channels_up =   [skip_n33u]*num_scales if isinstance(skip_n33u, int) else skip_n33u,
                                            num_channels_skip = [skip_n11]*num_scales if isinstance(skip_n11, int) else skip_n11, 
                                            upsample_mode=upsample_mode, downsample_mode=downsample_mode,
	...
	def forward(self, input):	
		...
		norm = torch.sqrt(0.00001*torch.ones((self.height,self.width)).type(self.dtype) + torch.maximum(torch.zeros((self.height,self.width)).type(self.dtype),norm_grad_red_squared + norm_grad_green_squared + norm_grad_blue_squared //
			+ (0.00001*torch.ones((self.height,self.width)).type(self.dtype) + torch.mul(red_x,green_y)-torch.mul(red_y,green_x)) + (torch.mul(blue_x,red_y)-torch.mul(blue_y,red_x)) + (torch.mul(green_x,blue_y)-torch.mul(green_y,blue_x)))) 
		norm = torch.unsqueeze(norm,dim=0)

		return output,norm	

class EuclideanDiracColor243B(nn.Module):
	def __init__(self,input_depth, dtype, pad, height, width, upsample_mode, n_channels=3, act_fun='LeakyReLU', skip_n33d=128, skip_n33u=128, skip_n11=4, num_scales=5, downsample_mode='stride'):
		super(EuclideanDiracColor243B,self).__init__()
		self.net = skip(input_depth, n_channels, num_channels_down = [skip_n33d]*num_scales if isinstance(skip_n33d, int) else skip_n33d,
                                            num_channels_up =   [skip_n33u]*num_scales if isinstance(skip_n33u, int) else skip_n33u,
                                            num_channels_skip = [skip_n11]*num_scales if isinstance(skip_n11, int) else skip_n11, 
                                            upsample_mode=upsample_mode, downsample_mode=downsample_mode,
...
class EuclideanDiracColor244B(nn.Module):
	def __init__(self,input_depth, dtype, pad, height, width, upsample_mode, n_channels=3, act_fun='LeakyReLU', skip_n33d=128, skip_n33u=128, skip_n11=4, num_scales=5, downsample_mode='stride'):
		super(EuclideanDiracColor244B,self).__init__()
		self.net = skip(input_depth, n_channels, num_channels_down = [skip_n33d]*num_scales if isinstance(skip_n33d, int) else skip_n33d,
                                            num_channels_up =   [skip_n33u]*num_scales if isinstance(skip_n33u, int) else skip_n33u,
                                            num_channels_skip = [skip_n11]*num_scales if isinstance(skip_n11, int) else skip_n11, 
                                            upsample_mode=upsample_mode, downsample_mode=downsample_mode,
	...
	def forward(self, input):	
		...
		norm = torch.sqrt(0.00001*torch.ones((self.height,self.width)).type(self.dtype) + torch.maximum(torch.zeros((self.height,self.width)).type(self.dtype),norm_grad_red_squared + norm_grad_green_squared + norm_grad_blue_squared //
			+ (0.00001*torch.ones((self.height,self.width)).type(self.dtype) + (torch.mul(green_x,blue_y)-torch.mul(green_y,blue_x)) ) + (torch.mul(blue_x,red_y)-torch.mul(blue_y,red_x)) + (torch.mul(red_x,green_y)-torch.mul(red_y,green_x)))) 
		norm = torch.unsqueeze(norm,dim=0)
