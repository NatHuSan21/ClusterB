...
for k in range(1,25):#25
    ...
    LAMval = [0]#,0.0001,0.0005,0.001,0.005,0.01]#,0.03,0.05]
    ...
    for lam in LAMval:
        ...
        ruta = './restoration/P270B/deblur_P270B_'
        fname = ruta + str(Lambda)  + nameimg 
        out_img_avg_pil.save(fname)

        #Save best image 
        imgPNSRmax_pil = np_to_pil(imgPNSRmax_np)
        fname = './restoration/P270B/BestPnsr_P270B_' + str(Lambda) + nameimg 
...
#name = nameimg.replace('.png','.csv')
name = 'KodimTotal.csv'
fname = ruta  + 'P270BdebMAXval' + name
