...
for k in range(2,3):#25
    ...
    for lam in LAMval:
        ...
        ruta = './restoration/P284B/deblur_P284B_'
        fname = ruta + str(Lambda)  + nameimg 
        out_img_avg_pil.save(fname)

        #Save best image 
        imgPNSRmax_pil = np_to_pil(imgPNSRmax_np)
        fname = './restoration/P284B/BestPnsr_P284B_' + str(Lambda) + nameimg 
    ...
    ruta = './txt284B/'
    name = nameimg.replace('.png','.csv')
    fname = ruta  + 'P284BdebMAXval' + name
